...
return $this->hasOne(Guest_resevation::class, 'user_id', 'id');
 ...
 /**
    ...
    {
        return Password::decrypt($this->login_password);
    }
    ...
    {
        $this->login_password = Password::encrypt($plainLoginPassword);
    }
...
// Event
  ...
  public function proseed()
  {
  return $this->hasOne(Proseed::class, 'event_id', 'id');
  }
  ...
  public function perfomance_list()
  {
  return $this->hasOne(PerfomanceList::class, 'event_id', 'id');
  }
    ...
    | getS〇〇〇〇List
    |
    | １ モデルで必要な情報を選別して取得する
    |
    | ２ コントローラーにuse文を追加して機能させます。
    |    $変数 = モデル名::メソッド名();
    |
    |
    | 3 一覧表示のindexメソッドなどに変数を入れて表示する
    | $〇〇List = MSection::get〇〇List();  //見本
    | データをビュー画面に引き渡す
    |        $requestData = compact(
            'ledger',
            'sectionList',
        ...
        return view('master.ledger.edit', $requestData);
    |
    |
    |
    |
    */
    ...
    /**
     * コントローラー内で変数をセットして、変数を利用して自身のモデルのカラムと一致させる
     * カラムを一致させた情報の関係する情報を取得する
     * リストとして一覧を取得して返す。
     *
     *  コントローラー見本
     *
     ...
     *
     *
     * このファンクションの（）の変数名はわかりやすい情報を記述
     * 特にどこのカラムを取得するかを明記するとわかりやすい
     *
    ...
    public static function getSectionMemberList($sectionId)
    {
        ...
        $list = self::where('section_id', $sectionId)->pluck('name','id');
        return $list;
    }
    ...
    /*
    |--------------------------------------------------------------------------
    | リレーションした情報から、表示を変更する
    |--------------------------------------------------------------------------
    |
    | リレーション表示系（外部キーを合わせて紐づいた情報を取得する）
    | ※モデルでリレーションを必ず設定しておくこと
    |
    |
    | 数字変換系
    |
    ...
    */
     ...
     *  ※既にモデルで外部キーでリレーションされている状態を設定しておきます。
     *
     *
     *  isset(Ⅰ) ? Ⅱ : Ⅲ;
     *  Ⅰが存在しているか確認し、Ⅰがある場合はⅡを出力、Ⅰがない場合は、Ⅲを出力
     * 　複数存在する場合は、&&で接続しておくこと。データベースにNULLが存在するとエラーになる。
     * 　isset($this->user) && isset($this->user->mTeam) ? $this->user->mTeam->name : null;
     *
     * 外部のモデルとリレーションを貼った時の設定方法
     * isset($this->リレーションのモデル名) ? $this->モデル->カラム名 : null;
     *
     * 数字表示系
     * 命名規則 Displayの頭文字の「Disp」を命名規則で利用する
     * isset($this->order_amount) ? number_format($this->order_amount) : 0;
     * isset($this->自分のモデルのカラム名) ? number_format($this->自分のモデルのカラム名) : 0;
     *
     */
    ...
    {
        return isset($this->user) && isset($this->user->mTeam) ? $this->user->mTeam->name : null;
    }
    /**
     * IDからイベント名を返す
     * 見本
...
public function proseeds()
{
return $this->hasMany(Proseed::class, ',ticket_rank_id', 'id');
}
...
public function perfomance_list()
{
return $this->hasOne(PerfomanceList::class, 'music_id', 'id');
}
