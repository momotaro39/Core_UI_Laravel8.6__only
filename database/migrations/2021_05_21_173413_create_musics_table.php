...
class CreateMusicsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('musics', function (Blueprint $table) {
            $table->id();
            $table->unsignedInteger('album_id')->comment('アルバムの紐付け');
            $table->unsignedInteger('band_id')->comment('バンドの紐付け');
            $table->string('name')->comment('曲名');
            $table->time('music_time')->comment('一曲の長さ');
            $table->integer('album_music_number')->comment('アルバムの中の収録順');
