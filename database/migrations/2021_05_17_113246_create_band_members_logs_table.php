...
class CreateBandMembersLogsTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('band_members_logs', function (Blueprint $table) {
            $table->id();
            $table->unsignedInteger('band_member_id')->comment('バンドメンバーのID');;
            $table->unsignedInteger('user_id')->comment('ログを入力した人のID');
            $table->text('log')->comment('来歴記録');
    ...
    {
        Schema::dropIfExists('band_members_logs');
    }
}
